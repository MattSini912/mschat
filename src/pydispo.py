...
FILE MODIFIED:
ALL PRINTS ARE COMMENTED
A SMALL CHANGE IN A RETURN VALUE IS HIGHLIGHTED
"""
...
def check_mailbox(email_addr,showInbox=True,showRecent=True):
    ...
    if showRecent:
        ...
        return check_single_email(email_addr,inboxmail_id = inboxmail_id_list[0])
    #print("#"*25)
    return;     
...
######################################################## MODIFIED HERE    
    ...
    d["id"] = response['id']
    d["to"] = email_addr
    d["from"] = response['from']
    d["date"] = response['date']
    d["subject"] = response['subject']#.decode("utf-8")
    d["attachments"] = str_attached
    return d
########################################################
    if printInTerminal:
        pass# <-------- HERE TOO
